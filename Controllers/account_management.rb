...
def create_account(account_file, tasks_file, firstname, lastname, username, password)
  ...
  random_id = rand(100_000)
  random_id = rand(100_000) while account_parsed[0][:created_id].include? random_id

  # append new id to created ids
  account_parsed[0][:created_id] << random_id

  # append new account to accounts.json
  account_parsed << { id: random_id, firstname: firstname, lastname: lastname, username: username,
                      password: password }
  ...
  User.new(random_id, firstname, lastname)
end
...
def validate_password(account_file, userid, password)
  parsed = JSON.load_file(account_file, symbolize_names: true)
  parsed.each do |user|
    return true if (user[:id] == userid) && (user[:password] == password)
  end
  false
end
...
def load_tasks(tasks_file, userid)
  ...
  parsed.each do |user|
    return user[:tasks] if user[:id] == userid
  end
end
