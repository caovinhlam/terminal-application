def login_menu(account_file, tasks_file, assigned_file)
  ...
  begin
    clear_commandline
    display_title('KANRI')
    menu_selection = prompt.select('What would you like to do?', ['Login', 'Create Account', 'Quit'],
                                   cycle: true).downcase
    case menu_selection
    when 'login'
      clear_commandline
    ...
    while menu_selection != 0
      ...
      when 2
        begin
          ...
          raise(ConfirmationError, 'Incorrect Password') unless validate_password(account_file, user_account.id,
                                                                                  password)
...
def my_task_menu(_account_file, tasks_file, user_account)
  prompt = TTY::Prompt.new(symbols: { marker: '>' })
  # Loading tasks from Database
  user_account.my_task.tasks = load_tasks(tasks_file, user_account.id) if user_account.get_tasks.empty?
    ...
    while menu_selection != 0
      ...
      when 1
        begin
          ...
          if !choices.empty?
            # Get index position from the hash to delete task in the array in user details
            user_index = prompt.select('Which user would you like to assign a task to?', choices,
                                       cyle: true)
